...
<header class = "menubar">
	...
	<section class="contentsection">
		...
		<span class="wizardbtn"><input type="button" class="wizard" value="마법사" onclick="location.href='magic_test.jsp'"></span>
		<div class="searchdiv">
...
// 		for (int i = 0; i < list.size(); i++) {
	...
	<%
		...
		%>
			...
			<div class="btndiv">
		<span class="createbtn"><input type="button" class="pluscreate" value="+작품생성" onclick="location.href='createcontent3.jsp'"></span>
	 ...
	 if(pageNumber != 1) {
			...
			<input type="button" class="beforea" value="이전" onclick="location.href=magic_content.jsp?pageNumber=<%=pageNumber -1 %>">
                
            <%
            	//BbsDAO에서 만들었던 함수를 이용해서, 다음페이지가 존재 할 경우
                } if (magicDAO.nextPage(pageNumber + 1)) {
            %>
            	<!-- a태그를 이용해서 다음페이지로 넘어 갈 수있는 버튼을 만들어 준다. -->
            	
            	<input type="button" class="nexta" value=">" onclick="location.href=magic_content.jsp?pageNumber=<%=pageNumber + 1 %>">
