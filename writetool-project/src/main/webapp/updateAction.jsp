...
<jsp:setProperty name="freetest" property="userID"/>
...
<body>
	<%
		...
		//성공적으로 권한이 있는사람이라면 넘어간다.
				...
				int result = freecontentDAO.update(freeID,number,Integer.parseInt(request.getParameter("roundID")),request.getParameter("subtitle"), request.getParameter("summary"),request.getParameter("story"));
				//오류가 발생했을때 띄워줄 메세지
				if (result == -1) {
					PrintWriter script = response.getWriter();
					script.println("<script>");
					...
					PrintWriter script = response.getWriter();
					script.println("<script>");
					script.println("location.href='view2.jsp?freeID="+freeID+"&number="+number+"'");
					//script.println("history.back()");
					script.println("</script>");
				}
			}
